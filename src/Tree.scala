class Tree () {
  ...
  class Node ( var data: Int, var count: Int, var left: Node, var right: Node, var parent: Node)

  var rootNode: Node = null
  var counter: Int = 0  //счетчик пройденных узлов

  def insert(value: Int, comparator: Comparator): Unit = {
  ...
  def search(p: Node, value: Int, comparator: Comparator): Node = {
    var res: Node = null
    if (null != p){
      counter = counter + 1
      if (p.data == value){
  ...
  def shift(value: Int, comparator: Comparator): Unit = {
